...
(function ( $ ) {
  ...
  function r_fun_ajax(fun, settings, handler){
    ...
    //ajax call
    var jqxhr = $.ajax(settings).done(function(responseBody,textStatus,xhr){
      var loc = xhr.getResponseHeader('Location') || console.log("Location response header missing.");
      var key = xhr.getResponseHeader('X-ocpu-session') || console.log("X-ocpu-session response header missing.");
      ...
      console.log("OpenCPU error HTTP " + xhr.status + "\n" + xhr.responseText);
    });
